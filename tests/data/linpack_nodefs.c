...
**                   indicate that dgesl or dgedi will divide by 0.0e0
        ...
        *info = 0;
        ...
        if (nm1 >=  0)
            for (k = 0; k < nm1; k++)
                ...
                /* 0.0e0 pivot implies this column already
                   triangularized */
...
**      job     integer
**              = 0         to solve  a*x = b ,
**              = non0.0e0   to solve  trans(a)*x = b  where
...
**   error condition
**
**      a division by 0.0e0 will occur if the input factor contains a
**      0.0e0 on the diagonal.  technically this indicates singularity
...
static void dgesl(REAL *a,int lda,int n,int *ipvt,REAL *b,int job,int roll)
    ...
    if (roll)
        ...
        if (job == 0)
            ...
            /* job = non0.0e0, solve  trans(a) * x = b  */
