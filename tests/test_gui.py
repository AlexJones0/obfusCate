...
class TestGuiComponents(unittest.TestCase):
    ...
    def test_file_namelabels(self) -> None:
        ...
        self.assertEqual(
            self.gui.obfuscated_namelabel.file_label.text(), "/obfuscated.c"
        )
    ...
    def test_click_obfuscate_button(self) -> None:
        ...
        source = CSource(
            os.path.join(os.getcwd(), "./tests/data/fibonacci_recursive.c")
        )
        self.gui.add_source(source)
        self.gui.selection_form.current_form.add_transform(
            obfs_gui.GuiIdentifierRenameUnit
        )
        QTest.mouseClick(
            self.gui.misc_form.general_options.obfuscate_button,
            Qt.MouseButton.LeftButton,
        )
        self.assertNotEqual(self.gui.obfuscated_editor.toPlainText(), "")
        self.assertNotEqual(
            self.gui.obfuscated_editor.toPlainText(),
            self.gui.source_editor.toPlainText(),
        )
    ...
    def test_add_transforms(self) -> None:
        ...
        for i, transform in enumerate(
            self.gui.selection_form.available_form.transforms
        ):
            widget = self.gui.selection_form.available_form.layout.itemAt(
                i + 1
            ).widget()
    ...
    def test_remove_transform(self) -> None:
        ...
        self.gui.selection_form.current_form.add_transform(obfs_gui.GuiReverseIndexUnit)
        ...
        QTest.mouseClick(
            self.gui.misc_form.transform_options.remove_button,
            Qt.MouseButton.LeftButton,
        )
        ...
        self.assertIsInstance(
            self.gui.selection_form.current_form.selected[0],
            obfs_gui.GuiIdentifierRenameUnit,
        )
        self.assertEqual(
            self.gui.selection_form.current_form.selected_widgets[0].class_,
            obfs_gui.GuiIdentifierRenameUnit,
        )
        self.assertFalse(self.gui.misc_form.transform_options.remove_button.isHidden())
        QTest.mouseClick(
    ...
    def test_metrics_loaded(self) -> None:
        ...
        self.assertEqual(
            self.gui.misc_form.metrics_form.metric_widget.layout.count(), 6
        )
        source = CSource(os.path.join(os.getcwd(), "./tests/data/minimal.c"))
        self.gui.add_source(source)
        self.gui.selection_form.current_form.add_transform(
        ...
        for i in range(1, 6):
            self.assertNotEqual(
                self.gui.misc_form.metrics_form.metric_widget.layout.itemAt(i),
                items[i - 1],
            )
    ...
    def test_disable_metric_group(self) -> None:
        ...
        for (
            metric_group,
            checkbox,
        ) in self.gui.misc_form.metrics_form.checkbox_map.items():
    ...
    def test_identifier_renaming_defaults(self) -> None:
        ...
        obfs_unit = obfs_gui.GuiIdentifierRenameUnit.get_gui()
        self.assertFalse(obfs_unit.minimise_idents)
        self.assertEqual(
            obfs_unit.style, obfs_gui.IdentifierRenameUnit.Style.COMPLETE_RANDOM
        )
        frame = QFrame()
    ...
    def test_index_reversal_defaults(self) -> None:
        ...
        customisation of the transform will result in a valid and expected obfuscation.
        Also checks these values are loaded into the option GUI widget correctly."""
        obfs_unit = obfs_gui.GuiReverseIndexUnit.get_gui()
    ...
    def test_whitespace_cluttering_defaults(self) -> None:
        ...
        customisation of the transform will result in a valid and expected obfuscation.
        Also checks these values are loaded into the option GUI widget correctly."""
        obfs_unit = obfs_gui.GuiClutterWhitespaceUnit.get_gui()
    ...
    def test_ditrigraph_encoding_defaults(self) -> None:
        ...
        customisation of the transform will result in a valid and expected obfuscation.
        Also checks these values are loaded into the option GUI widget correctly."""
        obfs_unit = obfs_gui.GuiDiTriGraphEncodeUnit.get_gui()
        self.assertEqual(obfs_unit.chance, 0.75)
        self.assertEqual(obfs_unit.style, obfs_gui.DiTriGraphEncodeUnit.Style.MIXED)
        frame = QFrame()
        obfs_unit.edit_gui(frame)
        obfs_unit.load_gui_values()
        self.assertEqual(obfs_unit.chance, 0.75)
    ...
    def test_string_encoding_defaults(self) -> None:
        ...
        customisation of the transform will result in a valid and expected obfuscation.
        Also checks these values are loaded into the option GUI widget correctly."""
        obfs_unit = obfs_gui.GuiStringEncodeUnit.get_gui()
        self.assertEqual(obfs_unit.style, obfs_gui.StringEncodeTraverser.Style.MIXED)
        frame = QFrame()
        obfs_unit.edit_gui(frame)
        obfs_unit.load_gui_values()
        self.assertEqual(obfs_unit.style, obfs_gui.StringEncodeTraverser.Style.MIXED)
    ...
    def test_arithmetic_encoding_defaults(self) -> None:
        ...
        customisation of the transform will result in a valid and expected obfuscation.
        Also checks these values are loaded into the option GUI widget correctly."""
        obfs_unit = obfs_gui.GuiArithmeticEncodeUnit.get_gui()
    ...
    def test_opaque_augmentation_defaults(self) -> None:
        ...
        customisation of the transform will result in a valid and expected obfuscation.
        Also checks these values are loaded into the option GUI widget correctly."""
        obfs_unit = obfs_gui.GuiAugmentOpaqueUnit.get_gui()
        self.assertEqual(obfs_unit.number, 1)
        self.assertEqual(obfs_unit.probability, 1.0)
        self.assertEqual(obfs_unit.styles, [s for s in obfs_gui.OpaqueAugmenter.Style])
        frame = QFrame()
        obfs_unit.edit_gui(frame)
        obfs_unit.load_gui_values()
        self.assertEqual(obfs_unit.number, 1)
    ...
    def test_opaque_insertion_defaults(self) -> None:
        ...
        customisation of the transform will result in a valid and expected obfuscation.
        Also checks these values are loaded into the option GUI widget correctly."""
        obfs_unit = obfs_gui.GuiInsertOpaqueUnit.get_gui()
        self.assertEqual(obfs_unit.number, 5)
        self.assertEqual(obfs_unit.styles, [s for s in obfs_gui.OpaqueInserter.Style])
        self.assertEqual(
            obfs_unit.granularities, [g for g in obfs_gui.OpaqueInserter.Granularity]
        )
        self.assertEqual(obfs_unit.kinds, [k for k in obfs_gui.OpaqueInserter.Kind])
        frame = QFrame()
        obfs_unit.edit_gui(frame)
        obfs_unit.load_gui_values()
        self.assertEqual(obfs_unit.number, 5)
    ...
    def test_control_flow_flattening_defaults(self) -> None:
        ...
        customisation of the transform will result in a valid and expected obfuscation.
        Also checks these values are loaded into the option GUI widget correctly."""
        obfs_unit = obfs_gui.GuiControlFlowFlattenUnit.get_gui()
        self.assertFalse(obfs_unit.randomise_cases)
        self.assertEqual(
            obfs_unit.style, obfs_gui.ControlFlowFlattener.Style.SEQUENTIAL
        )
        frame = QFrame()
        obfs_unit.edit_gui(frame)
        obfs_unit.load_gui_values()
    ...
    def test_integer_entry_invalid(self) -> None:
        ...
        _, entry = obfs_gui.generate_integer_widget(
            "...", "...", 4, -12, 16, parent_widget
        )
        entry.setText("")
    ...
    def test_float_entry_invalid(self) -> None:
        ...
        characters, not allowing their entry into the line edit widget."""
        parent_widget = QWidget()
        _, entry = obfs_gui.generate_float_widget(
            "...", "...", 4.56, -3.21, 10.502, parent_widget
        )
        entry.setText("")
        QTest.keyClicks(entry, "-1.2a6")
    ...
    def test_radio_button_selection(self) -> None:
        ...
        parent_widget = QWidget()
        _, buttons = obfs_gui.generate_radio_button_widget(
            "...", "...", {"a": 1, "b": 2, "c": 3}, "a", parent_widget
        )
    ...
    def test_checkboxes_selection(self) -> None:
        ...
        parent_widget = QWidget()
        _, checkboxes = obfs_gui.generate_checkboxes_widget(
            "...", "...", {"a": 1, "b": 2, "c": 3}, ["a", "b"], parent_widget
        )
